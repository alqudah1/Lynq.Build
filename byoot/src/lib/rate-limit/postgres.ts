...
// Copied verbatim from platform/src/lib/rate-limit/postgres.ts — the
// rate_limit_counters table shape in schema.ts is identical, so no
...
export class PostgresRateLimiter implements RateLimiter {
  ...
  async checkLimit(key: string, config: RateLimitConfig): Promise<RateLimitResult> {
    ...
    const row = rows[0];
    if (!row) {
      return { allowed: true, remaining: config.limit, resetAt: new Date(Date.now() + config.windowSeconds * 1000) };
    }
  ...
  /** A single atomic statement — increment-and-read, or reset-to-1 if the window expired — never a separate SELECT then UPDATE, which would race under concurrency. */
...
export async function deleteStaleRateLimitCounters(db: NeonHttpDatabase<Record<string, unknown>>): Promise<number> {
  ...
  const deleted = await db.delete(rateLimitCounters).where(lt(rateLimitCounters.windowStart, cutoff)).returning({ key: rateLimitCounters.key });
