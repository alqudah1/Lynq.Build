...
import { POST as ATTACH_TRUST } from "../trust/route";
...
async function makeUser(): Promise<string> {
  const [user] = await db.insert(users).values({ email: `brain-source-assignment-route-test-${crypto.randomUUID()}@example.com` }).returning({ id: users.id });
  createdUserIds.push(user.id);
  return user.id;
}

async function makeOrgWithOwner(ownerId: string): Promise<string> {
  const [org] = await db.insert(organizations).values({ name: "Source Assignment Route Org", slug: `brain-source-assignment-org-${crypto.randomUUID().slice(0, 8)}` }).returning({ id: organizations.id });
...
describe("GET .../versions/{versionNumber}/source-assignment", () => {
  ...
  it("returns 200 with isValid: false when no source has been recorded", async () => {
    const ownerId = await makeUser();
    const orgId = await makeOrgWithOwner(ownerId);
    await grantCapability(orgId, "identity", null, ownerId, "draft_write");
    await grantCapability(orgId, "identity", null, ownerId, "read");
  ...
  it("returns 200 with the correct rank once a source is recorded", async () => {
    const ownerId = await makeUser();
    const orgId = await makeOrgWithOwner(ownerId);
    await grantCapability(orgId, "identity", null, ownerId, "draft_write");
    await grantCapability(orgId, "identity", null, ownerId, "read");
    ...
    await authenticateAs(ownerId);
    await ATTACH_TRUST(
      new Request("https://platform.example.com/x", { method: "POST", body: JSON.stringify({ trustTier: "verified", expectedRevision: 0, sourceType: "founder_decision" }) }),
    ...
    expect(body.data.sourceType).toBe("founder_decision");
    expect(body.data.rank).toBe(1);
  });

  it("returns 404 for a cross-tenant version", async () => {
    const ownerId = await makeUser();
    const orgId = await makeOrgWithOwner(ownerId);
    await grantCapability(orgId, "identity", null, ownerId, "draft_write");
    await grantCapability(orgId, "identity", null, ownerId, "read");
    ...
    await authenticateAs(otherOwnerId);

    const res = await GET(new Request("https://platform.example.com/x"), { params: Promise.resolve({ organizationId: otherOrgId, knowledgeItemId: item.id, versionNumber: "1" }) });
