...
/** POST .../{executionId}/retry — `failed -> queued`, bounded by `maxRetries`, only for a transient failure class. */
export async function POST(_request: Request, { params }: RouteParams) {
  try {
    ...
    const execution = await retryExecution(db, { organizationId, executionId, actorUserId: user.userId });
