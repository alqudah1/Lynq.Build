...
import { salesApprovalLinks, marketingApprovalLinks, communicationApprovalLinks } from "@/db/schema";
...
export interface FounderApprovalItem extends AgentApprovalRequest {
  ...
  requestingSystem: "sales" | "marketing" | "communications" | "agent_runtime";
...
export async function listFounderApprovals(db: Db, input: { organizationId: string; actorUserId: string }): Promise<FounderApprovalItem[]> {
  ...
  const [salesLinks, marketingLinks, commsLinks] = await Promise.all([
