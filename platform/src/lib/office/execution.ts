 ...
 */
async function approvedActionArtifacts(db: Db, organizationId: string, projectId: string, requestedAction: string): Promise<Array<string | null>> {
  const rows = await db
    .select({ content: agentArtifacts.content })
...
async function hasApprovedAction(db: Db, organizationId: string, projectId: string, requestedAction: string): Promise<boolean> {
  return (await approvedActionArtifacts(db, organizationId, projectId, requestedAction)).length > 0;
}
 ...
 */
async function founderApprovedThisRestaurant(db: Db, organizationId: string, projectId: string, restaurantName: string): Promise<boolean> {
  const contents = await approvedActionArtifacts(db, organizationId, projectId, RESTAURANT_PROSPECT_APPROVAL_ACTION);
  return contents.some((content) => parseRestaurantResearch(content)?.recommendation.name === restaurantName);
}
...
export async function continueOfficeDirectiveExecution(db: Db, input: { organizationId: string; executionId: string }) {
  ...
  if (!artifact || latestApproval?.status === "revision_requested" || latestApproval?.status === "rejected") {
    if (metadata.stage === "research") {
      ...
      artifact = await createArtifact(db, {
        ...
        content: `${restaurantResearchMarker(research)}\n\n${renderRestaurantResearch(research)}`.slice(0, 20_000),
      ...
      if (!(await hasApprovedAction(db, input.organizationId, link.projectId, DEMO_APPROVAL_ACTION))) {
      ...
      if (prospect && !(await founderApprovedThisRestaurant(db, input.organizationId, link.projectId, prospect.recommendation.name))) {
      ...
      if (!delivery && latestApproval?.status === "revision_requested") {
        ...
        if (revisionProspect && !(await founderApprovedThisRestaurant(db, input.organizationId, link.projectId, revisionProspect.recommendation.name))) {
  ...
  if (metadata.stage === "qa") {
    ...
    const approval = await requestApproval(db, { organizationId: input.organizationId, executionId: execution.id, requestedAction: DEMO_APPROVAL_ACTION, summary: approvalSummary, riskLevel: "high", artifactId: artifact.id, proposedActionRef: { projectId: link.projectId, taskId: link.taskId }, actorAgentId: agent.id });
    await linkApprovalToEntity(db, { organizationId: input.organizationId, projectId: link.projectId, approvalRequestId: approval.request.id, linkedEntityType: "task", linkedEntityId: link.taskId, actorUserId: execution.ownerUserId });
    await notifyJarvisApprovalNeeded(db, { organizationId: input.organizationId, ownerUserId: execution.ownerUserId, projectId: link.projectId, projectName: projectRow.name, summary: approvalSummary });
    return approval.execution;
  }
