...
/** Development/log provider for SMS — used until a real Twilio credential is configured. See `dev-email.ts` for the "never falsely claim delivery" contract this mirrors exactly. */
export const devSmsProvider: ProviderAdapter = {
  provider: "dev_sms",
  channel: "sms",
  ...
  async verifyConnection() {
    return { verified: true, externalAccountId: "dev-sms-local", detail: "Development SMS provider — no real account, always verifies." };
  },

  async sendMessage(_credential, input: SendMessageInput): Promise<SendMessageResult> {
    const providerMessageId = `dev-sms-${randomUUID()}`;
    console.log(`[communications-os][dev-sms] would send to=${input.recipientReference} idempotencyKey=${input.idempotencyKey} providerMessageId=${providerMessageId}`);
